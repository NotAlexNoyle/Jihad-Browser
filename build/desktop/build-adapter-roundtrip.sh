...
# BrowserAdapter stand-in (YapClient), then run both under one Xvfb session:
# the client connects over the real YAP socket, sends Connect+OpenUrl, and
# receives the server->adapter messages (Load*, Painted). Proves the whole
# adapter<->server<->Goanna path with the contract unchanged.
set -uo pipefail
...
xvfb-run -a -s "-screen 0 1024x768x24" bash -c '
  ...
  dpid=$!
  sleep 7                      # let the engine come up + open the socket
