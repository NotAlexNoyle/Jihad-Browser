...
INCS="-include $DIST/include/mozilla-config.h -I$DIST/include -I$DIST/include/nspr -I/jihad/render/browserserver/Src $GTK_CFLAGS"
...
$CXX $CXXFLAGS $INCS -c "$SRC/test/tls_test.cpp"  -o /out/tls_test.o       || exit 12
$CXX /out/tls_test.o /out/GoannaRenderPage.o /out/EngineHost.o /out/DialogService.o /out/DownloadService.o \
...
echo "== generating self-signed cert + starting HTTPS server (127.0.0.1:18443) =="
CERTDIR=/out/tlscert; mkdir -p "$CERTDIR"
openssl req -x509 -newkey rsa:2048 -nodes -keyout "$CERTDIR/key.pem" \
  -out "$CERTDIR/cert.pem" -days 2 -subj "/CN=127.0.0.1" \
  -addext "subjectAltName=IP:127.0.0.1" >/dev/null 2>&1
python3 /jihad/render/goanna/test/tls_server.py 18443 "$CERTDIR/cert.pem" "$CERTDIR/key.pem" &
SRV=$!
sleep 1
JIHAD_TLS_BASE="https://127.0.0.1:18443" \
