...
| T-054 | DONE | 2026-07-18, commit 41d49f0. Machine confs tenderloin+opal (`build/webos-oe/conf/machine/`), shared jihad-touchpad.inc (DEFAULTTUNE armv7a-neon softfp), COMPATIBLE_MACHINE on engine+daemon recipes, UI recipes allarch. Finding: both models one softfp binary set (same SoC family, same 1024x768; only DPI differs 132 vs 183). docs/DEVICE-BUILD.md Topaz-vs-Opal section. Opal install + kernel string pending hardware [human-review on device] |
