...
| Offscreen Rendering (geometry) | **resize (R5) done** | 5 | GoannaRenderPage::Resize (nsIBaseWindow::SetPositionAndSize + GTK resize) reflows content to the new viewport; wired to BrowserPageGoanna::setWindowSize (bounded by the adapter's shm). Verified 1024×768→640×480, green content reflowed exactly (RESIZE PASS). Scroll/zoom (need JS-injection or non-frozen APIs) + contents-size/scrolled-to/meta-viewport events (R4) remain |
