...
| Navigation, Loading & Events | **history + setHtml** | 6 | load lifecycle msgs; back/forward with real canGoBack/canGoForward; setHtml inline — all verified (NAV PASS: A→B→Back(A)→Forward(B)→setHtml). Redirect rules + global-history + query-responses remain |
...
| Offscreen Rendering (geometry) | **R5 complete (resize+scroll+zoom)** | 5 | Resize→RESIZE PASS (1024×768→640×480 exact). ScrollTo (javascript: scrollTo) + GetScrollXY→SCROLL PASS (0,0)→(0,500). SetZoom (nsIContentViewer::SetFullZoom via a proper content-docshell lookup)→ZOOM PASS (1×=10000px→3×=90000px, 9×). All three wired to the daemon (setWindowSize/setScrollPosition/setZoomAndScroll). Side-fix: GetDocShell repaired SetJavaScriptEnabled's docshell path too. Only R4 events (contents-size/scrolled-to/meta-viewport) remain |
