...
| Navigation, Loading & Events | **history + setHtml + failed-load** | 6 | load lifecycle; back/forward (real canGo*); setHtml — NAV PASS. **R3 failed-load: msgFailedLoad on network failure (progress listener, ignoring NS_BINDING_ABORTED) + synchronous bad-URL rejection; FAIL-EVENT PASS (bad emits with URL, good stays clean).** Redirect events (R4) + history-state query (R5) + global-history/link-clicked (R6) remain |
