...
| Offscreen Rendering (geometry) | **resize + scroll (R5)** | 5 | Resize (nsIBaseWindow::SetPositionAndSize) reflows to the new viewport — RESIZE PASS (1024×768→640×480, exact). ScrollTo (javascript: window.scrollTo, no frozen scroll API) + GetScrollXY readback — SCROLL PASS (0,0)→(0,500); wired to daemon asyncCmdSetScrollPosition. Zoom (setZoomAndScroll) + R4 events (contents-size/scrolled-to/meta-viewport) remain |
