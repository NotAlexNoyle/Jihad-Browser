...
| 4 | high | **Removing either UI `.ipk` destroys the other's runtime** — both install the same `/media/internal/jihad/hl` + system shim + `/etc/event.d/jihad`; either `prerm` deletes all three. | 📝 kit + ⬜ (needs refcount or a shared runtime package / variant-specific paths) |
| 5 | high | **Shim impl-path hard-coded to the Enyo appid** — `BrowserAdapterShim.cpp` `kImplTrusted = .../net.riverstonerelay.jihad-browser/BrowserAdapterImpl.so`; Mochi/Mojo can't load their own impl. | 📝 kit + ⬜ (per-variant shim path, or a shared system-owned impl; C++ change) |
...
| 9 | medium | **Patch + runtime-closure failures hidden** — `patch … \|\| true`; unresolved `DT_NEEDED` printed not fatal; NSS/GRE optional copies. | ⬜ (fail on patch reject; add a required-file manifest + closure check) |
