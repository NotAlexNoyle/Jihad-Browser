...
- [~] Enable / disable / remove work from the UI and the change is reflected after a restart. *(2026-08-04: **enable and disable work from the UI** — clicking them runs the add-on's `shutdown()`/`startup()` and the page effect follows (R4); the list also re-sorts enabled-first, so its own state is reflected. NOT yet exercised: **remove** from the UI, and a restart taken specifically after a UI toggle.)*
