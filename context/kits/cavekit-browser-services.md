...
- [~] Accepting proceeds with the load; rejecting aborts it. *(2026-08-04 — REJECT is verified, ACCEPT is blocked, and the blocker is now identified rather than suspected. Against a real self-signed TLS server, the cert error raises the SSL-confirm dialog with host and code, the card's answer comes back correctly either way (`ssl-confirm -> ACCEPT` / `-> cancel`), and on reject the load stays failed — that half is done. On accept, `AcceptCurrentCert` needs an `nsIX509Cert` for `RememberValidityOverride` and **there is no certificate object to give it**. Two independent sources both come up empty: (1) `nsIBadCertListener2::NotifyCertProblem` — the only place a cert was ever captured — NEVER FIRES in this embedding, because it is reached through the channel's notification callbacks and nothing routes those to our chrome for a document load; the `mCertError` flag that made this look wired is set by a SECOND path, the load-failure handler, which infers a cert error from the nsresult module alone and has no cert. (2) Added here as the standard alternative (it is how Firefox's own cert-error page gets the cert): the failed channel's `securityInfo` does answer `nsISSLStatusProvider` (`secInfo=1 prov=1`) but `GetSSLStatus` returns NULL (`status=0`), so the handshake aborted before an `nsISSLStatus` was ever attached. The capture code is left in place — it is correct and costs nothing — and the daemon now logs `secInfo/prov/status/cert` at the failure, which is the only place that distinction is visible. Next step for whoever picks this up: find why NSS produces no SSLStatus for an overridable error here (`CreateCertErrorRunnable` in `security/manager/ssl` is where it should be attached), not to add a third capture site.)*
