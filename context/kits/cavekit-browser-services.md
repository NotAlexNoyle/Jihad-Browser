...
**Remaining:** route `msgSSLConfirm` through the same FIFO (it still passes an empty pipe path, so a certificate prompt cannot be answered), and confirm the round trip on device with a real tap — the card ends are built in all three variants.
