...
- [x] `prerm` removes exactly the files this package created and leaves everything else — verified by a full filesystem diff across an install→remove cycle showing no residue and no collateral deletion. *(2026-08-01 — VERIFIED ON DEVICE across the full three-variant matrix (`device-independence-test.sh matrix`), with control scripts invoked as the supported Preware/WOQI path runs them. After installing Enyo → Mochi → Mojo and removing all three, `device-citizen-audit.sh diff baseline post-matrix2` reported **IDENTICAL** — zero residue, nothing collaterally deleted, all stock checksums unchanged. The snapshot was hardened first (review F-14): it now walks `/media/internal` recursively rather than top-level, and lists every entry under `/usr/lib/jihad` and `/var/palm/jihad` rather than filtering by type, so a leftover empty directory or a residual file cannot hide from it.)*
