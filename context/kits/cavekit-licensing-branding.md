...
**Acceptance Criteria:**
- [x] Files derived from the Apache-2.0 upstreams (UI, IPC daemon) retain their original Apache-2.0 headers unmodified.
- [x] New engine-backend files carry an MPL-2.0 header.
- [x] No upstream license header is removed or altered.
**Dependencies:** none

### R2: Accurate top-level LICENSE and NOTICE
**Description:** The repository documents its composite licensing and attributions.
**Acceptance Criteria:**
- [x] `LICENSE` enumerates each component, its directory, and its license.
- [x] `NOTICE` attributes HP, LG, and Mozilla/Moonchild as required.
- [x] Full Apache-2.0 and MPL-2.0 texts are included under `licenses/`.
**Dependencies:** none
...
**Acceptance Criteria:**
- [x] The engine build is configured to remove Pale Moon/Basilisk branding (name, about pages, logos, default UA token).
- [x] Shipped artifacts present only the "Jihad Browser" product name.
- [x] A scan of shipped artifacts finds no "Pale Moon"/"Basilisk"/"Moonchild" branding strings.
...
**Acceptance Criteria:**
- [x] Any modification to an MPL-2.0 file stays under MPL-2.0 and its source is available.
- [x] The engine source origin and any patches are documented.
...
**Acceptance Criteria:**
- [x] `LICENSE` states how the Apache-2.0 and MPL-2.0 parts combine (file-level copyleft, compatible).
**Dependencies:** none
...
- 2026-07-04: Status reconciled to implementation — all R1–R5 done: headers, LICENSE/NOTICE, branding strip (0 strings in shipped libxul/daemon/goanna.js), MPL source docs, compatibility.
