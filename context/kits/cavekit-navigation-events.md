...
- [x] A link activated by tap/click (not only an engine content-initiated load) emits a link-clicked message with the target URL — verified by `link_test`. *(**2026-08-04 — the MESSAGE is now observed, which is what this criterion asks for and what every earlier record was missing.** A tap on a relative-href anchor logs `linkClicked -> card: http://127.0.0.1:8731/target.html` (the href RESOLVED to an absolute URL, as the card needs) and the navigation then completes to that page with `back=1`. `msgLinkClicked` had no log line of its own until now, so the emission was invisible even when it happened; it has one at both call sites. **A trap this exposed, for anyone writing the next test:** the `clickid`/`dblclickid`/`clickoff` inject commands call `GoannaRenderPage::ClickAt` DIRECTLY and therefore skip the input QUEUE — and `TakeClickNav` is consumed in `pump()`, so those commands never produce a link-clicked message and never re-drive the navigation. The first run of this test showed exactly that: the anchor hit-tested, the engine started a load, it ended `NS_BINDING_ABORTED`, and nothing was emitted. Use `click <x> <y> <n>` (card coordinates, the real path) for anything about navigation semantics; `clickid` and friends are for reaching chrome controls.)* *(Historical detail preserved: *(review #5 H-2: the tap-interception path bypasses the engine's programmatic-load classifier, so the message must be emitted at interception.)* *(T5 ROOT FOUND + FIXED 2026-07-20 (2be6d85): "link taps show the loading overlay but nothing appears / overloads the browser" was the daemon CRASHING. A synchronous nav started from inside pump() (tapped link/button re-drive, Enter-submit) spun the engine's nested event loop, which re-entered tick() and re-pumped the SAME page mid-navigation → SIGBUS (GoannaRenderPage::BeginLoad, this==0xa); upstart respawned → the app saw the socket drop = stuck full-card overlay. Fixed with an `if (mInTick) return;` re-entrancy guard in tick(). Verified on device: the google/ddg tap+Enter sequence that gave 36 daemon restarts now gives ZERO. Also resolves the un-clickable search suggestions and html.ddg "still loading". [~] pending a full user re-test of link-tap navigation. See context/impl/device-test-2026-07-19.md.)* *(2026-07-31 reconciliation: link-tap navigation HAS since been confirmed on device — commit d4f0842 (2026-07-27) records "a tap on example.com's 'Learn more' link now hit-tests the anchor and navigates", after the zoom coordinate-consistency fix that was the second cause of missed taps (cavekit-input-bridging.md R5, `../impl/zoom-fix-2026-07-27.md`). Stays `[~]` because that record confirms the NAVIGATION, not this AC's actual subject — that the interception path EMITS the link-clicked message with the target URL — and no broad real-site re-test is recorded.)*
...
- 2026-07-31: Reconciliation against recorded evidence (no box changed state). R6's link-clicked AC keeps `[~]`, but its note now records that on-device link-tap navigation was confirmed 2026-07-27 (commit d4f0842) after the zoom coordinate-consistency fix — while stating why that does not close this AC (it evidences the navigation, not the link-clicked message emission, and no broad real-site re-test is recorded).
