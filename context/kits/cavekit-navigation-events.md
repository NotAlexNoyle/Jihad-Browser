...
**Acceptance Criteria:**
- [ ] `openUrl` loads the URL.
- [ ] `back`/`forward`/`reload`/`stop` perform the expected navigation.
- [ ] `canGoBack`/`canGoForward` state is accurate and reflected in location messages.
- [ ] `clearHistory` empties session history.
...
**Acceptance Criteria:**
- [ ] `setHtml(url, body)` renders the provided body as the document at the given base URL.
**Dependencies:** none

### R3: Load lifecycle events in correct order
**Description:** Loads produce an ordered, complete event stream.
**Acceptance Criteria:**
- [ ] A normal load emits load-started, then progress updates from 0 to 100, then load-stopped and document-load-finished.
- [ ] A failed load emits a failed-load / main-document-error message with domain, code, failing URL, and description.
- [ ] `stop` mid-load ends the stream cleanly.
...
**Acceptance Criteria:**
- [ ] Navigation emits a location-changed message with the URL and back/forward state.
- [ ] Title resolution emits title-changed (and the combined title-and-url message where upstream did).
- [ ] Redirects emit a url-redirected message.
...
**Acceptance Criteria:**
- [ ] `getHistoryState(queryNum)` produces a history-state response carrying the same query number and correct back/forward flags.
**Dependencies:** cavekit-ipc-contract.md (R1)

### R6: Global history and link-click reporting
**Description:** Side-channel navigation events are reported.
**Acceptance Criteria:**
- [ ] Navigation emits an update-global-history message.
- [ ] Intercepted link activations emit a link-clicked message.
- [ ] URL-redirect rules registered via `addUrlRedirect` are honored.
