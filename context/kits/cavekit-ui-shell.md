...
- [ ] Find-in-page issues `findInPage`. *(The UI half is correct — `Browser.js` calls `callBrowserAdapter("findInPage", …)` from the FindBar, and the command reaches the daemon. What is missing is BELOW that: `GoannaRenderPage::Find` deliberately does not call `nsIWebBrowserFind::FindNext`, because it faults in this offscreen configuration — it dereferences a frame-selection controller the offscreen browser does not set up. **RE-TESTED 2026-08-04** in case the note had gone stale after the offscreen widget gained real event dispatch/focus/editing: it has not — enabling FindNext still SIGSEGVs (`find_test` exit 139). So find-in-page cannot work until an offscreen-safe selection controller is wired, which is engine work, not a UI test. This AC was previously described as "the smallest open item"; that was wrong.)*
