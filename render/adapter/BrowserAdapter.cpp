 ...
 *   pinch-zoom blit, scroll indicator) and single-tap -> asyncCmdClickAt dispatch,
 *   ported from the Atlas Browser project (Herrie82, Apache-2.0). See ../../NOTICE. */
...
void BrowserAdapter::handlePaint(NpPalmDrawEvent* event)
{
    ...
    if (!info || info->renderedWidth <= 0 || info->renderedHeight <= 0)
        return;
    ...
    int sl = event->srcLeft, st = event->srcTop;
    int panY = mScrollPos.y, panX = mScrollPos.x;      // pan within the tall buffer
    const unsigned int FILL = 0xffffffff;              // un-rendered area: white, not stale (ghosting)

    // Pinch/zoom: buffer holds the page at info->contentZoom; display at mZoomLevel. Scale by
    // invScale (nearest-neighbour, fixed-point 16.16). invScale==1 => exact 1:1 blit (normal browsing).
    double czoom   = (info->contentZoom > 0.0) ? info->contentZoom : 1.0;
    double invScale = (mZoomLevel > 0.0) ? (czoom / mZoomLevel) : 1.0;
    const int FP = 16;
    int32_t stepFp = (int32_t)(invScale * (double)(1 << FP) + 0.5);
    int baseX = sl - info->renderedX;
    int baseY = st - info->renderedY;

    for (int dy = dt; dy < dbm; ++dy) {
        int64_t cyFp = (int64_t)((dy - dt) + panY) * stepFp;
        int oy = baseY + (int)(cyFp >> FP);
        unsigned int* drow = (unsigned int*)(dstBase + (dy - dt) * dstStride);
        if (oy < 0 || oy >= info->renderedHeight) {    // whole row outside the buffer -> fill
            for (int dx = dl; dx < dr; ++dx) drow[dx - dl] = FILL;
            continue;
        }
        const unsigned int* srow = (const unsigned int*)(srcBase + oy * srcStride);
        int64_t cxFp = (int64_t)(panX) * stepFp;
        for (int dx = dl; dx < dr; ++dx) {
            int ox = baseX + (int)(cxFp >> FP);
            drow[dx - dl] = (ox < 0 || ox >= info->renderedWidth) ? FILL : srow[ox];
            cxFp += stepFp;
        }
    }

    // Scroll indicator (the QPainter scrollbar is gone with the dstBuffer path): stock-style
    // dark bar on the right edge, blended over the content, fading via mScrollbarOpacity.
    if (mScrollbarOpacity > 0 && mContentHeight > (int)mWindow.height && (dbm - dt) > 0) {
        int alpha = mScrollbarOpacity > 0xD0 ? 0xD0 : mScrollbarOpacity;
        int vh = dbm - dt;
        const int barW = 6, margin = 8;
        int thumbH = (int)((double)mWindow.height / (double)mContentHeight * vh);
        if (thumbH < 50) thumbH = 50;
        int thumbY = (int)((double)mScrollPos.y / (double)mContentHeight * vh);
        if (thumbY + thumbH > vh) thumbY = vh - thumbH;
        if (thumbY < 0) thumbY = 0;
        int x0 = (dr - dl) - barW - margin;
        if (x0 < 0) x0 = 0;
        for (int yy = thumbY; yy < thumbY + thumbH && yy < vh; ++yy) {
            unsigned int* drow = (unsigned int*)(dstBase + yy * dstStride);
            for (int xx = x0; xx < x0 + barW && xx < (dr - dl); ++xx) {
                unsigned int p = drow[xx];
                int r = (p >> 16) & 0xFF, g = (p >> 8) & 0xFF, b = p & 0xFF;
                r = (r * (255 - alpha) + 0xAA * alpha) / 255;
                g = (g * (255 - alpha) + 0xAA * alpha) / 255;
                b = (b * (255 - alpha) + 0xAA * alpha) / 255;
...
bool BrowserAdapter::handleGesture(NpPalmGestureEvent *event)
{
    ...
    case npPalmGestureSingleTapEvent:
        {
            // Dispatch a click so links/buttons/fields activate on a tap. The pen path only
            // forwards a mousedown when the tap hits a known interactive rect, but those
            // rects are never reported by the Goanna daemon, so single taps did nothing.
            // Convert the tap (window px, minus the chrome header) to CONTENT coords -- the
            // asyncCmdClickAt contract is contentX/contentY = (scroll + event) / zoom.
            double z = (mZoomLevel > 0.0) ? mZoomLevel : 1.0;
            int32_t cx = (int32_t)((mScrollPos.x + event->x) / z);
            int32_t cy = (int32_t)((mScrollPos.y + (event->y - m_headerHeight)) / z);
            asyncCmdClickAt(cx, cy, 1, 1);
        }
        break;
    }
